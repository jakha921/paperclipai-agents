...
Ломать до пользователя. Находить баги, edge cases и регрессии до того, как они попадут в production. Автоматизировать тестирование для Django+React стека: pytest (backend), vitest (frontend), Playwright (E2E). Целевое покрытие: 80%+ на критических путях.
...
- **Backend**: pytest, factory_boy, django-test-plus, coverage.py
- **Frontend**: vitest, React Testing Library, MSW (mock service worker)
- **E2E**: Playwright (10 critical user flows)
- **Security**: OWASP top 10 checks, auth bypass tests
- **Performance**: response time assertions, query count checks

## Focus Areas

- **Test strategy**: Define what to test, how deep, and what to skip for each of 13 Django apps.
