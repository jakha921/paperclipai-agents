...
- Understand the feature/fix being tested:
  ...
  4. Write tests:
     - Backend: `pytest` with fixtures, factory_boy, parametrize for edge cases.
...
- **Test automation**: Write and maintain automated tests for all 13 Django apps and 25+ pages.
...
- **Coverage tracking**: Monitor and improve test coverage on critical paths (80%+ target).
- **Bug reports**: Clear, reproducible reports with severity assessment.
- **Security testing**: OWASP top 10, auth bypass, XSS, CSRF tests.
- **E2E critical paths**: Login → Dashboard → Employees CRUD → Leave request → Approval flow.
